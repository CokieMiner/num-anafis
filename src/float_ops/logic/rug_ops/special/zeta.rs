...
use super::{BackingFloat, get_precision};
...
use crate::int_math::from_i64;
use alloc::vec;
use rug::float::Constant::Pi;
use rug::float::Special::Nan;
...
pub fn zeta_deriv(n: &IntType, v: &BackingFloat) -> BackingFloat {
    ...
    let Some(n_usize) = n.to_usize() else {
        return Float::with_val(get_precision(), Nan);
    };
    ...
    let two_pi = Float::with_val(work_prec, &two * Float::with_val(work_prec, Pi));
    ...
    let pi_half = Float::with_val(work_prec, Float::with_val(work_prec, Pi) / 2);
    ...
    for m in 1..=n_usize {
        let psi_val = polygamma(
            &from_i64(i64::try_from(m - 1).expect("fits")).expect("m-1 fits"),
            &one_minus_s,
        );
...
fn exp_linear_series_rug(base: &Float, slope: &Float, order: usize, prec: u32) -> Vec<Float> {
    let mut coeffs = vec![Float::with_val(prec, 0); order + 1];
...
fn reciprocal_series_rug(series: &[Float], order: usize, prec: u32) -> Vec<Float> {
    let mut out = vec![Float::with_val(prec, 0); order + 1];
...
fn rising_factorial_series_rug(s: &Float, factors: usize, order: usize, prec: u32) -> Vec<Float> {
    ...
    for j in 0..factors {
        let mut next = vec![Float::with_val(prec, 0); order + 1];
...
fn eta_taylor_rug(order: usize, s: &Float, prec: u32) -> Vec<Float> {
    ...
    let mut d_coeffs = vec![Float::with_val(prec, 0); n + 1];
    ...
    let mut numerator = vec![Float::with_val(prec, 0); order + 1];
...
fn borwein_taylor_rug(order: usize, s: &Float, prec: u32) -> Vec<Float> {
    ...
    let mut denom = vec![Float::with_val(prec, 0); order + 1];
...
fn zeta_deriv_internal(n: usize, s: &Float, prec: u32) -> Float {
    ...
    if delta.clone().abs() < tenth {
        ...
        let mut c_coeffs = vec![Float::with_val(prec, 0); order_eta + 1];
        let mut f_coeffs = vec![Float::with_val(prec, 0); order_eta + 2];
        ...
        let mut h_coeffs = vec![Float::with_val(prec, 0); order_eta + 1];
        for k in 0..=order_eta {
        ...
        let mut g_coeffs = vec![Float::with_val(prec, 0); order + 1];
        ...
        for j in 0..(order - n) {
            ...
            let delta_j = Float::with_val(prec, Pow::pow(delta.clone(), &Integer::from(j)));
