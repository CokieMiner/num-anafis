...
use rug::float::Special::Infinity;
use rug::ops::Pow;
...
pub fn lgamma(value: &BackingFloat) -> BackingFloat {
    if value.is_sign_negative() {
        if value.is_integer() {
            ...
            return Float::with_val(prec, Infinity);
        }
        let prec = get_precision();
...
pub fn trigamma(value: &BackingFloat) -> BackingFloat {
    polygamma(&from_i64(1).expect("1 fits in IntType"), value)
}

pub fn tetragamma(value: &BackingFloat) -> BackingFloat {
    polygamma(&from_i64(2).expect("2 fits in IntType"), value)
}
...
pub fn polygamma(n: &IntType, value: &BackingFloat) -> BackingFloat {
    ...
    while x < shift {
        let term = Float::with_val(work_prec, Pow::pow(x.clone(), &neg_np1));
        s += term;
        x += &one;
    }
    ...
    // Leading: (n-1)! / xⁿ
    let pow_x_n = Float::with_val(work_prec, Pow::pow(x.clone(), &n_int));
    ...
    let pow_x_n1 = Float::with_val(work_prec, Pow::pow(x.clone(), &n1));
    ...
    for k in 1..=max_k {
        ...
        let pow_val = Float::with_val(
            work_prec,
            Pow::pow(x.clone(), Integer::from(ni) + Integer::from(2 * k)),
        );
...
pub(in crate::float_ops) fn bernoulli_even_up_to(max_k: usize) -> Vec<rug::Rational> {
    let mut b = vec![Rational::from((1, 1)), Rational::from((-1, 2))];
    for m in 2..=(2 * max_k) {
        if m % 2 != 0 {
            b.push(Rational::from((0, 1)));
            continue;
        }
    ...
    let mut evens = Vec::with_capacity(max_k);
    for k in 1..=max_k {
