...
impl CliffordNumber {
    ...
    pub(crate) fn make_zero_coeffs(n: u8) -> CliffordCoeffs {
        ...
        if n <= INLINE_GENERATOR_LIMIT {
            ...
            CliffordCoeffs::Heap(vec![z; coeff_count_unchecked(n)])
        }
    }
    ...
    pub(crate) fn reembed(&self, new_gens: GeneratorSet, perm: &[u8]) -> Self {
        ...
        if new_n <= INLINE_GENERATOR_LIMIT {
            let arr: [Scalar; INLINE_COEFF_COUNT] = array::from_fn(|i| {
                if i < new_count {
                    buf[i].clone()
                } else {
                    z.clone()
...
impl CliffordNumber {
    ...
    ///
    /// # Errors
    /// [`NumAnafisError::InlineCoefficientsRequireAtMostFiveGenerators`] if n > 5.
    /// [`NumAnafisError::ActiveGeneratorsTooLargeForPlatform`] if the generator count
    /// overflows `u8` or 2ⁿ overflows `usize`.
    pub fn from_coeffs(
        gens: GeneratorSet,
