...
impl Cmplx {
    ...
    pub fn apply_real_only(&self, f: &impl Fn(&Scalar) -> Scalar) -> Self {
        ...
        if self.1.abs().total_cmp(&eps) == Ordering::Greater {
...
pub fn can_embed(gens: &GeneratorSet) -> bool {
    ...
    gens.len() + num_nilpotent <= MAX_EMBED_GENS
}
...
pub fn compute_blade_basis(gens: &GeneratorSet) -> Option<Vec<Mat2C>> {
    ...
    for pos in 0..n {
        ...
        if metric == 1 {
            gen_mats.push(pauli_matrix(pauli_idx));
            pauli_idx += 1;
        } else if metric == -1 {
            gen_mats.push(pauli_matrix(pauli_idx).scale(&Cmplx::i()));
            pauli_idx += 1;
        } else {
            let g1 = pauli_matrix(pauli_idx);
            let g2 = pauli_matrix(pauli_idx + 1);
